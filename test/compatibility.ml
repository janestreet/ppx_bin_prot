...
module Common = struct
  type tuple = float * string * int64 [@@deriving bin_io, equal, sexp_of]
  ...
  type 'a singleton_record = { y : 'a } [@@deriving bin_io, equal, sexp_of]
  ...
  type 'a poly_id = 'a rec_t1 [@@deriving bin_io, equal, sexp_of]
  type el = float poly_id [@@deriving bin_io]
  type els = el array [@@deriving bin_io]

  module Wildcard : sig
    type _ transparent = int [@@deriving bin_io, equal, sexp_of]
    type _ opaque [@@deriving bin_io, equal, sexp_of]

    val opaque_examples : int opaque list
  end = struct
    type _ transparent = int [@@deriving bin_io, equal, sexp_of]
    type 'a opaque = 'a option [@@deriving bin_io, equal, sexp_of]
