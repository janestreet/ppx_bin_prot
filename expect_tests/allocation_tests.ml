...
let%expect_test "No allocation when writing variants" =
  ...
  let _ : int = show_allocation (fun () -> writer.write buf ~pos:0 No_arg) in
  let one_arg = Test.One_arg "foo" in
  let () = [%expect {|
    (allocated
      (minor_words 0)
      (major_words 0)) |}] in
  let _ : int = show_allocation (fun () -> writer.write buf ~pos:0 one_arg) in
  let () = [%expect {|
    (allocated
      (minor_words 0)
      (major_words 0)) |}] in
  let record_arg = Test.Record_arg { num = 5 } in
  let _ : int = show_allocation (fun () -> writer.write buf ~pos:0 record_arg) in
  [%expect {|
    (allocated
      (minor_words 0)
      (major_words 0)) |}];
